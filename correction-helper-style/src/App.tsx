      ...
      <main className="ml-72 flex-1 p-8 grid grid-cols-4 grid-rows-[auto_1fr_1fr] gap-6">
        ...
        <div className="col-span-2 row-span-2 bento-card overflow-hidden relative">
          <div className="flex justify-between items-start mb-10">
            <span className="px-3 py-1 bg-indigo-50 text-brand-primary text-[10px] font-bold uppercase tracking-widest rounded-full border border-indigo-100">Live Status</span>
          ...
          <div className="mt-auto space-y-6">
            ...
            <div className="bg-slate-50 rounded-[1.5rem] p-4 flex items-center justify-between border border-slate-100">
              <div className="flex gap-2">
                ...
                <div className="w-8 h-8 rounded-full border-2 border-white bg-indigo-100 flex items-center justify-center text-[10px] font-bold text-brand-primary shadow-sm">+8</div>
              </div>
...
function StatusDot({ active = false, completed = false }: { active?: boolean, completed?: boolean }) {
  return (
    <div className={`h-2 rounded-full transition-all duration-500 ${
      ...
      active ? 'bg-brand-primary w-full shadow-[0_0_12px_rgba(79,70,229,0.4)]' : 
      'bg-slate-200 w-full'
    }`} />
  );
}
